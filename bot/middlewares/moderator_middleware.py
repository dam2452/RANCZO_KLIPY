...
class ModeratorMiddleware(BotMiddleware):
    async def handle(
        ...
        if event.from_user.username and await self._does_user_have_moderator_privileges(event.from_user.username):
            return await handler(event, data)

        await event.answer("❌ Nie masz uprawnień moderatora.❌")
        self._logger.warning(f"Unauthorized moderator access attempt by user: {event.from_user.username}")
