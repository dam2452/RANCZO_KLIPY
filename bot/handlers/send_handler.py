...
class SendClipHandler(BotMessageHandler):
    ...
    async def _do_handle(self, message: Message) -> None:
        ...
        if os.path.getsize(temp_file_path) == 0:
            await self.__reply_empty_file_error(message, clip_name, temp_file_path)
            return
        ...
        await self._log_system_message(logging.INFO, f"Clip '{clip_name}' sent to user '{username}' and temporary "
                                                     f"file removed.")
    ...
    async def __reply_empty_file_error(self, message: Message, clip_name: str, temp_file_path: str) -> None:
