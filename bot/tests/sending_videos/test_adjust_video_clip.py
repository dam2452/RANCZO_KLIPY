...
class TestAdjustClipCommand(BaseTest):
    ...
    async def test_adjust_clip_with_two_params(self):
        await self.assert_command_result_file_matches(await self.send_command('/klip geniusz'), 'geniusz.mp4')
        await self.assert_command_result_file_matches(await self.send_command('/dostosuj -5.5 1.5'), 'geniusz_adjusted_-5.5_1.5.mp4')
    ...
    async def test_adjust_clip_with_three_params(self):
        ...
        await self.assert_command_result_file_matches(await self.send_command('/wybierz 1'), 'kozioł_clip_1.mp4')
        await self.assert_command_result_file_matches(await self.send_command('/dostosuj 1 10.0 -3'), 'kozioł_adjusted_10.0_-3.mp4')
    ...
    async def test_adjust_clip_with_large_extension_values(self):
        await self.assert_command_result_file_matches(await self.send_command('/klip geniusz'), 'geniusz.mp4')
        await self.assert_command_result_file_matches(await self.send_command('/dostosuj 50 50'), 'geniusz_adjusted_50_50.mp4')
