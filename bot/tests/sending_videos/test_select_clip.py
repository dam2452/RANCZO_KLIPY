...
class TestSelectClipCommand(BaseTest):
    ...
    async def test_select_multiple_clips_in_sequence(self):
        await self.expect_command_result_contains('/szukaj kozioł', ["Wyniki wyszukiwania"])
        await self.assert_command_result_file_matches(await self.send_command('/wybierz 1'), 'kozioł_clip_select_1.mp4')
        await self.assert_command_result_file_matches(await self.send_command('/wybierz 2'), 'kozioł_clip_select_2.mp4')
