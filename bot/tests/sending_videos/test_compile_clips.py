...
class TestCompileClipsCommand(BaseTest):
    ...
    async def test_compile_all_clips(self):
        ...
        await self.assert_file_matches(await self.send_command('/kompiluj wszystko', timeout=30), 'Anglii_compilation_all.mp4')

    @pytest.mark.quick
    async def test_compile_clip_range(self):
        await self.expect_command_result_contains('/szukaj kozioł', ["Wyniki wyszukiwania"])
        await self.assert_file_matches(await self.send_command('/kompiluj 1-4'), 'kozioł_compilation_1-4.mp4')

    @pytest.mark.quick
    async def test_compile_specific_clips(self):
        await self.expect_command_result_contains('/szukaj kozioł', ["Wyniki wyszukiwania"])
        await self.assert_file_matches(await self.send_command('/kompiluj 1 3 5'), 'kozioł_compilation_1_3_5.mp4')
    ...
    async def test_compile_invalid_clip_numbers(self):
        await self.expect_command_result_contains('/szukaj kozioł', ["Wyniki wyszukiwania"])
        await self.assert_file_matches(await self.send_command('/kompiluj 1 abc 3'), 'kozioł_compilation_1_abc_3.mp4')
